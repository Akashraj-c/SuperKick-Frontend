...
import { addCartApi, getAllMensProductApi } from '../../services/allApi';
...
const MenProducts = () => {
    ...
    // get All Sneakers
    const getAllMensProduct = async () => {
        const result = await getAllMensProductApi(searchKey)
        // console.log(result);
        if (result.status == 200) {
            setAllMensProduct(result.data)
    ...
    const filterButton = (data) => {
        ...
        if (data == 'L-H') {
            setAllMensProduct([...tempArray].sort((a, b) => a.price - b.price))
        }
        else if (data == 'H-L') {
            setAllMensProduct([...tempArray].sort((a, b) => b.price - a.price))
        }
        else {
            setAllMensProduct(tempArray)
        }
    }
    ...
    return (
        <>
            ...
            <div style={{ marginTop: '140px', userSelect: 'none' }} >
                <p className='px-5 d-lg-flex d-none' style={{ color: 'rgba(94, 89, 89, 0.53)', fontSize: '14px' }}><Link to={'/'} className='text-decoration-none' style={{ color: 'rgba(94, 89, 89, 0.53)' }}>HOME</Link> / MENS </p>
            </div>
